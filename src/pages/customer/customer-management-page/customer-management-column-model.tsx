...
const keyInfosArray: any = () => {
  return [
    {key: colKey.company, label: 'Company Name'},
...
const CustomerManagementColumnModel = (props: any): any => {
  const getOtherInfoRestFormItems = (rowData: any) => {
    return [
  ...
  modelArr.push(
    {
      ...getColModelItem(colInfos.otherInfo, keyInfosArray),
      render: (rowData: any) => {
        const itemsObj = getItemsObj(keyInfosArray(), rowData)
        const keywords = colInfos.otherInfo.keywords
        const restFormItems = getOtherInfoRestFormItems(rowData)
        return renderFn(itemsObj, keywords, restFormItems)
      },
    },
    {
      title: 'Created At',
