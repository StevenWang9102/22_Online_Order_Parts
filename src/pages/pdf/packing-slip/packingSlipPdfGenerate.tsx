import pdfMake from 'pdfmake/build/pdfmake';
import vfsFonts from 'pdfmake/build/vfs_fonts';

export default (data:any, getBlob:any) => {
    // console.log(data)
    const {vfs} = vfsFonts.pdfMake;
    pdfMake.vfs = vfs;

    const contents = table(data)
    // const data1 = table1(20);
    // const formattedData = _format(data1);

    const documentDefinition:any = {
        pageSize: 'A4',
        pageOrientation: 'portrait',
        pageMargins : [30, 30, 30, 30],

        // Page Layout

        content :[
            {

                // This table will contain ALL content
                table : {
                    // Defining the top 2 rows as the "sticky" header rows
                    headerRows: 1,
                    // One column, full width
                    widths: ['*'],
                    heights: [80, 100, 50, 20, 80],
                    body: [


                        // Header Row One
                        // An array with just one "cell"
                        [
                            // Just because I only have one cell, doesn't mean I can't have
                            // multiple columns!
                            {
                                columns : [
                                    {
                                        image:'data:image/png;base64,',
                                        width: 160, height: 40,
                                    },
                                    {
                                        type: 'none',
                                        ul: [
                                            { text: 'PACKING SLIP', alignment: 'right',bold: true, fontSize:19, color: 'red'},
                                            { text: `No: ${data.quoteDate? data.quoteDate: ' '}`, alignment: 'right' },
                                            { text: `ORDER NO: ${data.customer.orderNo? data.customer.orderNo: ' '}`, alignment: 'right' },
                                            { text: `DATE: ${data.customer.orderDate? data.customer.orderDate.slice(0,10): ' '}`, alignment: 'right' },
                                        ]
                                    },
                                ]
                            }
                        ],


                        // Second Header Row

                        [
                            {
                                columns: [
                                    {
                                        type: 'none',
                                        ul: [
                                            ' ',
                                            { text: 'JADCUP'},
                                            { text: '4 Pukekiwiriki Pl, East Tamaki, Auckland 2013,'},
                                            { text: 'New Zealand'},
                                            { text: 'info@jadcup.co.nz'},
                                            { text: '092823988'},
                                        ]
                                    },

                                    {
                                        type: 'none',
                                        ul: [
                                            { text: 'TO:Customer', alignment: 'right' },
                                            { text: `${data.customer.customer.company? data.customer.customer.company: ' '}`, alignment: 'right' },
                                            { text: `${data.customer.customer.address? data.customer.customer.address: ' '}`, alignment: 'right' },
                                            { text: `${data.customer.customer.country? data.customer.customer.country: ' '}`, alignment: 'right' },
                                            { text: `${data.customer.customer.email? data.customer.customer.email: ' '}`, alignment: 'right' },
                                            { text: `${data.customer.customer.phone? data.customer.customer.phone: ' '}`, alignment: 'right' },
                                        ]
                                    }
                                ]
                            }
                        ],
                        [
                            {
                            table : {
                                widths: ['*'],
                                body: [
                                    [{text:'Delivery', bold:true, fillColor: '#CCCCCC'}],
                                    [{ text: `${data.customer.deliveryAddress? data.customer.deliveryAddress: ' '}` }],
                                    [' '],
                                    ['Bank: ANZ'],
                                    ['Account Number:020-2103-32023']
                                ],
                            },
                            layout: 'noBorders',
                            }
                        ],
                        [
                            {
                                style: 'tableExample',
                                table: {
                                    widths: [100, 250, '*','*'],
                                    body: [
                                        [{text:'ITEM NO', bold:true},{text:'DESCRIPTION', bold:true},{text:'NO. CARTON', bold:true},{text:'QTY', bold:true}],
                                        ...contents,
                                        // ...formattedData
                                    ]
                                },
                                layout:{
                                    fillColor: function (rowIndex:any){
                                        return (rowIndex === 0)?  '#CCCCCC': null
                                    }

                                }
                            }
                        ],
                        [
                            {
                                headlineLevel: 'Conditions',
                                table : {
                                    widths: ['*'],
                                    body: [
                                        [{text:'Terms and Conditions of Sales', bold:true, fillColor: '#CCCCCC'}],
                                        [' '],
                                        ['Bank: ANZ'],
                                        ['Account Number:020-2103-32023']
                                    ],
                                },
                                layout: 'noBorders',

                            },
                        ],
                        [
                            {
                                columns:[
                                    {text: 'Issuer:',width:100},
                                    {columns:[
                                            {text: 'Client Signature:'},
                                            {text: ' \t\t\t\t\t\t\t\t\t\t\t\t', decoration: 'underline',width:40}
                                        ],
                                        width:280
                                    },
                                    {columns:[
                                            {text: 'Date:'},
                                            {text: ' \t\t\t\t\t\t\t\t\t', decoration: 'underline',width:10}
                                        ],
                                        // width:300
                                    },
                                ]
                            },
                        ]
                    ]
                },
                layout: 'noBorders',
                margin: [ 0, 0, 0, 20 ]


                // Table Styles

                // layout: {
                //     hLineWidth: function(i, node) { return (i === 1 || i === 2) ? 1 : 0; },
                //     vLineWidth: function(i, node) { return 0; },
                //     hLineColor: function(i, node) { return (i === 1 || i === 2) ? '#eeeeee' : 'white'; },
                //     vLineColor: function(i, node) { return 'white' },
                //     paddingBottom: function(i, node) {
                //         switch (i) {
                //             case 0:
                //                 return 5;
                //             case 1:
                //                 return 2;
                //             default:
                //                 return 0;
                //         }
                //     },
                //     paddingTop: function(i, node) {
                //         switch (i) {
                //             case 0:
                //                 return 0;
                //             case 1:
                //                 return 2;
                //             default:
                //                 return 10;
                //         }
                //     }
                // }
            },
            // {text: 'my text!!!!!', absolutePosition: {x: 100, y: 700}}
        ],


        // Page Footer

        footer : function(currentPage:any, pageCount:any) {
            return {
                alignment : 'center',
                text      : currentPage.toString() + ' of ' + pageCount,
                fontSize  : 8
            }
        },


        pageBreakBefore: function(currentNode:any, followingNodesOnPage:any, nodesOnNextPage:any, previousNodesOnPage:any) {
            console.log('currentNode',currentNode,'followingNodesOnPage',followingNodesOnPage,'previousNodesOnPage',previousNodesOnPage)
            return currentNode.headlineLevel === 'Conditions' && currentNode.startPosition.top > 720
        },
    };

    pdfMake.createPdf(documentDefinition).open();
    const pdfDocGenerator = pdfMake.createPdf(documentDefinition);
    pdfDocGenerator.getBlob((blob) => {
        // console.log(blob,'blob')
        getBlob(blob)
    });
}

const table = (data:any ) =>{
    return data.tableContent.map((res:any) =>{
        console.log([res.product, res.description, res.carton, res.quantity])
        return [res.product, res.description, res.carton, res.quantity]
    })

}

//for test
const table1 = (data:any ) =>{
    let io = []
    for(let i =0; i< data; i++){
        io.push({product:`inini${i}`, description:`adsdada${i}`, carton:`${i}`,quantity:`${i}`})
    }
// console.log(io)
    return io

}

const _format = (data:any) => {
    console.log(data)
    return data.map((item:any) => {
        return [item.product,
             item.description,
             item.carton,
            item.quantity
        ];
    });
}
